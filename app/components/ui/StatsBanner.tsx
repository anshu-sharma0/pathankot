...
export function StatsBanner({ 
  ...
  if (isMinimal) {
    return (
      <section className={`bg-white border-b border-slate-100 ${className}`}>
          ...
          <div className={`grid gap-4 sm:gap-6 ${gridCols}`}>
            {stats.map((stat, idx) => (
              <div key={idx} className="text-center p-2 sm:p-3">
                ...
                <p className="text-[10px] sm:text-xs text-slate-400 mt-0.5 uppercase tracking-wider">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className={`grid gap-4 sm:gap-6 ${gridCols}`}>
        {stats.map((stat, idx) => (
          <div key={idx} className="p-5 rounded-2xl border border-slate-100 bg-slate-50 text-center">
            ...
            <p className="font-bold text-slate-800 text-sm uppercase tracking-wide mb-2">{stat.label}</p>
            {stat.desc && <p className="text-xs text-slate-500">{stat.desc}</p>}
          </div>
        ))}
      </div>
    </Card>
