          ...
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden animate-fade-in-up origin-top">
            {categories.map((cat) => (
              <button
                ...
                className={`w-full flex items-center justify-between px-5 py-3.5 text-left font-semibold transition-colors border-b border-slate-100 last:border-b-0 ${activeCategory === cat ? 'bg-teal-50 text-teal-700' : 'text-slate-600 hover:bg-slate-50'
                  }`}
              >
                {cat}
